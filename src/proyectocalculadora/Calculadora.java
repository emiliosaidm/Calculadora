...
public class Calculadora {
    ...
    // que estas que definimos fueran correctas: https://regex101.com/
     ...
     /**
     * <pre>
     * La siguiente clase proporciona la funcionalidad de realizar operacaciones
    ...
    /**
     * Llama a la función de validación, si es válida llama a la funcíon para pasarla a postija y, 
     * finalmente, llama al metodo de evaluación.
     * @param expresion Dada por el usuario.
     * @return String[] <ul>
     *                   <li> Si la expresión es erronea, el índice 0 del arreglo tiene un "0".</li>
     *                   <li> Si la expresión es correcta, el índice 1 del arreglo tiene un "1"; el índice 1 invluye el resultado de la evaluación </li>
     *                  </ul>
     * @see validez, pasarAPostfija, evaluarExpresion
     **/
    ...
    /**
     * Pasa a postijo la expresión matematica previamente validada.
    ...
    private double evaluarExpresion(ArrayList <String> postfija){
        ...
        for(i = 0; i < postfija.size(); i++){
            ...
            // De otra manera, se toman los 2 elementos de la pila de operandos, se toma el operador del elemento que, al no ser un numero, es un operador, se
        ...
        // El resultado de la operación se toma del tope de la pila, en caso de no ser vacia. 
        if(!pila.isEmpty()){
            resp = pila.pop();
        }
    ...
    private ArrayList<String> separarExpresion(String texto){
        // Se define una Expresión Regular para identificar numeros reales y operadores.
        // Se utiliza la clase de Patrones para complilar la expresión regular y pasarsela a la clase Matcher para matchear los patrones del Regex en el String.
        ...
        for(int i = 0; i < resultado.size(); i++){
            // El siguiente condicional determina que en caso de haber 2 terminios consecutivos sin un operador entre ambos, entonces se debe de agregar una suma.
            // Esto ocurre si dos numeros consecutivos se restan. El signo se resta lo conserva el elemento porque el Regex asi lo detecta, pero al operarlo se requiere de una suma. 
            if(i < resultado.size() - 1 && resultado.get(i).matches("^[-+]?\\d*\\.?\\d+$") && resultado.get(i + 1).matches("^[-+]?\\d*\\.?\\d+$")){
                resultado.add(i + 1, "+");
            // El siguiente condicional determina que en caso de haber una cerradura de parentesis y un operando, entonces se debe de agrear una suma, por la misma razón 
            // de la condicional de arriba.
            } else if(i < resultado.size() - 1 && resultado.get(i).equals(")") && resultado.get(i + 1).matches("^[-+]?\\d*\\.?\\d+$")){
               resultado.add(i + 1, "+"); 
            }
            // El siguiente condicional, en caso de haber un numero negativo, a menos que el operador previo sea la apertura de un parentesis, una multiplicacion o una división, intercambia
            // el signo negativo del digito, lo vuelve positivo y agrega un operador negativo a la cadena, con la finalidad de facilitar la evaluación
    ...
    private int jerarquia(String operador){
        ...
        // Devuelve la jerarquia de cada operador.
        return resp;
    }
    ...
    public static void main(String[] args) {
       String[] resp;
        ...
        resp = calculadora.calcularExpresion("3^2+3/-2");
        if(resp[0].equals("1")){
